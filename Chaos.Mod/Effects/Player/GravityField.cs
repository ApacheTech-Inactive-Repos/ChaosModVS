...
{
    public class GravityField : ChaosEffect
    {
        ...
        public override void OnServerTick(float dt)
        {
            AsyncEx.Server.EnqueueAsyncTask(() =>
            {
                var entities = ApiEx.Server.World.GetEntitiesAround(
                    Player.Entity.ServerPos.AsBlockPos, Settings["ScanDistance"].AsInt(50)).ToList();
                foreach (var entity in entities)
                {
                    // Inputs.
                    var startDistance = Settings["ForcefieldRadius"].AsInt(50);
